...
const GOOGLE_LIVE_SINGLE_KEY = "ENCLAWS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ENCLAWS_LIVE_ANTHROPIC_KEY",
    listVar: "ENCLAWS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ENCLAWS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ENCLAWS_LIVE_${base}_KEY`;
