...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "ENCLAWS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("ENCLAWS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv ENCLAWS_GATEWAY_TOKEN");
    expect(message).not.toContain("ENCLAWS_GATEWAY_PASSWORD");
  });
