...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".enclaws");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ENCLAWS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ENCLAWS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via ENCLAWS_STATE_DIR.
